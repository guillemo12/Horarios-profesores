(() => {
  ...
  function initCalendar() {
    ...
    AppData.calendarInstance.on("beforeUpdateEvent", async function(info) {
      ...
      if (constituentClasses.length === 2) {
        ...
        const end1 = new Date(newStartDate.getTime() + 30 * 6e4);
        ...
        const end2 = new Date(newStartDate.getTime() + 60 * 6e4);
        ...
        cls.duration = (endCandidateDate.getTime() - newStartDate.getTime()) / (1e3 * 60 * 60);
  ...
  function getMergedCalendarEvents(classes, type, entityId, colorMode) {
    ...
    groupsMap.forEach((groupClasses) => {
      ...
      while (i < groupClasses.length) {
        ...
        const isContiguous = next !== null && Math.abs(currentEnd - nextStart) < 6e4;
        const currentDur = current.duration || (new Date(current.end).getTime() - new Date(current.start).getTime()) / 36e5;
        const nextDur = next ? next.duration || (new Date(next.end).getTime() - new Date(next.start).getTime()) / 36e5 : 0;
  ...
  function getBestAssetForPlatform(assets, userAgent = typeof navigator !== "undefined" ? navigator.userAgent : "", platform = typeof navigator !== "undefined" ? navigator.platform : "") {
  ...
  function isDevEnvironment(win = typeof window !== "undefined" ? window : null) {
    if (!win) return true;
  ...
  async function exportDatabase() {
    try {
      ...
      let filename = `EduSchedule_Backup_${(/* @__PURE__ */ new Date()).toISOString().split("T")[0]}.db`;
