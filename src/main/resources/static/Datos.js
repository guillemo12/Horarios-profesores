...
function initCalendar() {
  ...
  AppData.calendarInstance.on("beforeUpdateEvent", async function(info) {
    ...
    if (changes.start)
      cls.start = typeof changes.start.toDate === "function" ? changes.start.toDate() : new Date(changes.start);
    if (changes.end)
      cls.end = typeof changes.end.toDate === "function" ? changes.end.toDate() : new Date(changes.end);
...
function refreshCalendarView() {
  ...
  const events = AppData.scheduledClasses.filter((cls) => {
    if (type === "teacher")
      return cls.teacherId === entityId;
    if (type === "group")
      return cls.groupId === entityId;
    return false;
  }).map((cls) => {
  ...
  if (AppData.calendarInstance)
    AppData.calendarInstance.createEvents(events);
}
function openEventDetail(event) {
  ...
  if (headerEl)
    headerEl.style.backgroundColor = teacher.color;
  const bodyEl = document.getElementById("event-detail-body");
  if (bodyEl)
    bodyEl.innerHTML = `<p class="text-sm">Impartida por: <b>${teacher.name}</b></p>`;
...
function openGroupModal(courseId, groupId = null) {
  ...
  const titleEl = document.getElementById("crud-modal-title");
  if (titleEl)
    titleEl.textContent = groupId ? "Editar Grupo" : "Nuevo Grupo";
...
window.toggleAvailabilitySlot = function(day, start, end, cellId) {
...
function loadSettings() {
  ...
  const elHuecosPuntos = document.getElementById("settings-huecos-puntos");
  const elCompactarPuntos = document.getElementById("settings-compactar-temprano-puntos");
  if (elMinimo)
    elMinimo.value = conf.tiempoMinimo.toString();
  if (elMaximo)
    elMaximo.value = conf.tiempoMaximo.toString();
  if (elMaxProfe)
    elMaxProfe.value = conf.minutosMaximosProfesor.toString();
  if (elPriorizar) {
    ...
    elPriorizar.onchange = () => {
      if (container)
        container.style.display = elPriorizar.checked ? "flex" : "none";
    };
  }
  if (elPriorizarPuntos)
    elPriorizarPuntos.value = conf.priorizarTutorPuntos.toString();
  if (elBloquesPuntos)
    elBloquesPuntos.value = conf.fomentarBloques60Puntos.toString();
  if (elHuecosPuntos)
    elHuecosPuntos.value = conf.evitarHuecosPuntos.toString();
  if (elCompactarPuntos)
    elCompactarPuntos.value = conf.compactarTempranoPuntos.toString();
}
...
function setupWebSocketsListeners() {
  ...
  AppData.WS.on("connected", () => {
    ...
    if (textBtn)
      textBtn.textContent = "Generar (WS)";
    if (wsStatus) {
  ...
  AppData.WS.on("disconnected", () => {
    ...
    const textBtn = document.getElementById("text-engine-btn");
    if (textBtn)
      textBtn.textContent = "Conectando...";
    if (wsStatus) {
  ...
  AppData.WS.on("scores_updated", (scores) => {
    ...
    if (elHard)
      elHard.textContent = scores.hard.toString();
    if (elSoft)
      elSoft.textContent = scores.soft.toString();
...
function toggleOptimizationEngine(forceStop = false) {
  try {
    ...
    if (AppData.WS.isOptimizing || forceStop) {
      ...
      if (textEngineBtn)
        textEngineBtn.textContent = "Generar (WS)";
    } else {
      ...
      if (textEngineBtn)
        textEngineBtn.textContent = "Parar Motor";
    }
  } catch (err) {
...
function switchTab(tabId) {
  ...
  if (tabId === "calendar") {
    setTimeout(() => {
      if (AppData.calendarInstance)
        AppData.calendarInstance.render();
      updateEntitySelector2();
      updateDateRange();
    }, 50);
  }
...
function updateEntitySelector2() {
  ...
  const entitySelect = document.getElementById("view-entity-select");
  if (!typeSelect || !courseSelect || !entitySelect || !courseSeparator)
    return;
  const type = typeSelect.value;
